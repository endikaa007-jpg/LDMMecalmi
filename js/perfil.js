...
function capitalizarPais(texto = '') {
    return String(texto || '').toLowerCase().split(' ').filter(Boolean).map((palabra) => palabra.charAt(0).toUpperCase() + palabra.slice(1)).join(' ');
}
...
function asegurarDatalistPaises(paises = []) {
    ...
    datalist.innerHTML = '';
    Array.from(new Set((paises || []).filter(Boolean))).sort((a, b) => a.localeCompare(b, 'es')).forEach((pais) => {
...
function poblarSelectPaises(paises = [], paisSeleccionado = '') {
    ...
    const ordenados = Array.from(setPaises).sort((a, b) => a.localeCompare(b, 'es'));
...
function seleccionarPaisEnSelect(paisSeleccionado = '') {
    ...
    const existe = Array.from(campoPais.options).some((opt) => opt.value === paisSeleccionado);
    if (!existe) {
        const option = document.createElement('option');
...
async function cargarPaisesEnSelect() {
    try {
        ...
        const paises = Array.isArray(data) ? data.map((item) => item?.name?.common).filter(Boolean) : [];
        if (paises.length === 0) {
            poblarSelectPaises(PAISES_FALLBACK, usuarioActual?.pais || '');
            return;
        }
        poblarSelectPaises(paises, usuarioActual?.pais || '');
    } catch (error) {
        poblarSelectPaises(PAISES_FALLBACK, usuarioActual?.pais || '');
    }
}
...
async function subirAvatarCloudinary(file) {
    ...
    if (!String(file.type || '').startsWith('image/')) throw new Error('Solo imagen permitida.');
    ...
    const data = await res.json();
    if (!res.ok || !data?.secure_url) throw new Error(data?.error?.message || 'Error al subir a Cloudinary.');
...
$(document).ready(function () {
    ...
    $('#editAvatarArchivo').on('change', function () {
        ...
        if (!archivo) {
            $('#editAvatarPreview').attr('src', usuarioActual?.avatarUrl || usuarioActual?.urlavatar || AVATAR_FALLBACK);
            return;
        }
        const urlLocal = URL.createObjectURL(archivo);
        $('#editAvatarPreview').attr('src', urlLocal);
    });

    $('#btnMostrarEdicion').click(function () {
        $('#editNombre').val(usuarioActual.nombre || usuarioActual.usuario || '');
        $('#editEmail').val(usuarioActual.email || '');
        seleccionarPaisEnSelect(usuarioActual.pais || '');
    ...
    $('#btnGuardarCambios').click(async function () {
        ...
        if (nombre && nombre.length < 3) return mostrarAlerta('El nombre debe tener al menos 3 caracteres.', 'error');
        if (email && !email.includes('@')) return mostrarAlerta('El email no es válido.', 'error'); 
        ...
        try {
            const archivoAvatar = document.getElementById('editAvatarArchivo')?.files?.[0] || null;
            if (archivoAvatar) {
